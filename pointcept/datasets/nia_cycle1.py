...
class NiaCycle1Dataset(DefaultDataset):
    def __init__(
        self,
        split="train",
    ...
    def get_label_path_from_lidar_path(self, lidar_path):
        collections_path = Path(self.data_root) / 'collections'
        annotations_path = Path(self.data_root) / 'annotations'
    ...
    def get_split_data_list(self, split):
        ...
        label_paths = list(map(self.get_label_path_from_lidar_path, lidar_paths))

        # A sample of data_list is a tuple: (lidar_path, label_path)
        all_data_list = list(zip(
            map(str, lidar_paths),
            map(str, label_paths),
        ))

        # # Currently, this creates custom splits with randomly shuffled data
        # split_data_dict = dict(zip(
        #     ["train", "val", "test"],
        #     split_by_ratio(all_data_list, ratio=[8, 1, 1], random_seed=230927),
        # ))

        split_data_dict = dict(zip(
            ["train", "val", "test"],
            split_by_yanghoon(all_data_list)
        ))

        return split_data_dict[split]
    ...
    def get_data_name(self, idx):
        ...
        collections_path = Path(self.data_root) / 'collections'
        data_name = Path(lidar_path).relative_to(collections_path).with_suffix('')
        return data_name
